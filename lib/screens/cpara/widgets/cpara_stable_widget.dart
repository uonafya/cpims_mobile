...
class _CparaStableWidgetState extends State<CparaStableWidget> {
  ...
  RadioButtonOptions? getOverallOption(
      {RadioButtonOptions? question1Option,
      RadioButtonOptions? question2Option,
      RadioButtonOptions? question3Option}) {
    if (question1Option == null &&
        question2Option == null &&
        question3Option == null) {
      return null;
    } else {
  ...
  void initState() {
    StableModel stableModel =
        context.read<CparaProvider>().stableModel ?? StableModel();
    question1Option = stableModel.question1 == null
        ? question1Option
        : convertingStringToRadioButtonOptions(stableModel.question1!);
    question2Option = stableModel.question2 == null
        ? question2Option
        : convertingStringToRadioButtonOptions(stableModel.question2!);
    question3Option = stableModel.question3 == null
        ? question3Option
        : convertingStringToRadioButtonOptions(stableModel.question3!);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return StableWidgetWrapper(
      children: [
        const GoalWidget(
          title:
              'Stable: Goal 5: Increase Caregiver’s Ability to Meet Important Family Need',
          description:
              'Benchmark 5: Caregivers are able to access money (without selling productive assets) to pay for school fees,'
        ...
        QuestionWidget(
          question:
              "5.1 Were you able to pay school fees for the last two terms for all school going children in your household without PEPFAR support? (Confirm availability of school fees receipt, government supported cash transfer/scholarships, confirm school going children are retained in school)",
          selectedOption: (value) {
            setState(() {
              ...
              context.read<CparaProvider>().updateStableModel(
                  stableModel.copyWith(question1: selectedOption));
              //invoke recalculation of the benchmark score
            });
          },
          isNaAvailable: true,
        ...
        QuestionWidget(
          question:
              "5.2 Was anyone sick in the past six months, were you able to pay all medical costs in the past 6 months for all children in your household under the age of 18 without PEPFAR support? Medical costs include medicine and transport to medical appointments",
          selectedOption: (value) {
            setState(() {
              ...
              String selectedOption =
                  convertingRadioButtonOptionsToString(value);
              context.read<CparaProvider>().updateStableModel(
                  stableModel.copyWith(question2: selectedOption));
              //invoke recalculation of the benchmark score
            });
          },
          isNaAvailable: true,
        ...
        QuestionWidget(
          question:
              "5.3 In case you find yourself in a situation, are you currently able to pay for legal and other administrative fees related to guardianship, civil registration or inheritance?",
          selectedOption: (value) {
            setState(() {
              ...
              String selectedOption =
                  convertingRadioButtonOptionsToString(value);
              context.read<CparaProvider>().updateStableModel(
                  stableModel.copyWith(question3: selectedOption));
              //invoke recalculation of the benchmark score
            });
          },
          isNaAvailable: false,
        ...
        BenchMarkAchievementWidget(
          text: "Has the household achieved this benchmarks?",
          benchmarkOption: getOverallOption(
...
class BenchMarkAchievementWidget extends StatelessWidget {
  ...
  const BenchMarkAchievementWidget(
      {super.key, required this.text, required this.benchmarkOption});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: FutureBuilder<List<CPARADatabase>>(
        future:
            database != null ? getUnsyncedForms(database!) : Future.value([]),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return pastCparaList.isEmpty
                ? const NotFoundWidget(
                    text: 'No past CPARA found',
                  )
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return PastCPARACardWidget(
                        cparaDatabase: cparaDatabase,
                      );
                    });
          } else {
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  overallBenchMarkScoreValueFromDb(
                      cparaDatabase: cparaDatabase),
                  style: const TextStyle(
                      fontWeight: FontWeight.bold,
                      fontSize: 20,
...
class BenchmarkScoreWidget extends StatelessWidget {
  ...
  const BenchmarkScoreWidget(
      {super.key, required this.index, required this.cparaDatabase});
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Text(
          '(${overallBenchMarkScoreFromDb(index: index + 1, cparaDatabase: cparaDatabase)})',
          style: const TextStyle(
...
class BenchmarkCustomRadioButtons extends StatelessWidget {
  ...
  const BenchmarkCustomRadioButtons({required this.benchmarkOption, super.key});
...
String benchMarkTwoScoreFromDb({required CPARADatabase cparaDatabase}) {
  ...
  String overallQuestion1Answer =
      "Yes"; // Is there anyone who is HIV positive in the Household ?
  String overallQuestion2Answer =
      "Yes"; // Is there a child 0 - 12 years who is HIV positive ?
  String overallQuestion3Answer =
      "Yes"; // Is there an adolescents or a child above 12 years who is HIV positive ?
...
String benchMarkFourScoreFromDb({required CPARADatabase cparaDatabase}) {
  ...
  String overallQuestion1Answer =
      "Yes"; // Is there child < 5 years in the household ?
  String overallQuestion2Answer =
...
String benchMarkSixScoreFromDb({required CPARADatabase cparaDatabase}) {
  ...
  String overallQuestion1Answer =
      "Yes"; // Are there children, adolescents, and caregivers in the household who have experienced violence
  String overallQuestion2Answer =
      "Yes"; // Is there adolescents 12 years and above ?
  String childQuestion1Answer =
...
String overallBenchMarkScoreFromDb(
    {required int index, required CPARADatabase cparaDatabase}) {
  switch (index) {
    case 1:
...
String overallBenchMarkScoreValueFromDb(
    {required CPARADatabase cparaDatabase}) {
  int score = 0;

  List<String> answers = [
    benchMarkOneScoreFromDb(cparaDatabase: cparaDatabase),
